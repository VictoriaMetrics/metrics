...
func (sm *Summary) metricType() string {
	// this metricsType should not be printed, because quantiles of the same metric family will be printed first,
	// and if metadata is needed, the metadata from quantiles should be used.
	// summary (count and sum) will be printed later, so its metrics type won't be printed as metadata.
